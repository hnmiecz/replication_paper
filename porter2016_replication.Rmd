...
####Import data
#data from qualtrics will be in CSV format
pilot = read.csv("psych 251_pilot.csv")
...
pilot_long = pilot %>%
  gather("question", "response",
         -c(participant, participant_pol, gender,
  ...
  separate(condition, into=c("LIB_condition", "POL_condition"),
           sep=" ~~~ ") %>%
  mutate(question = ifelse(question == "friendship_likelihoo",
                           "friendship_likelihood",
                           question)) %>%
  mutate(question = paste(question, "_response", sep="")) %>%
...
```{r}
manip_check_rude.aov = lm(peter_rude_response ~ LIB_condition*POL_condition,(pilot_long %>% filter()))
